package com.royalenfield.diagcan.Iso14229UdsClient.UdsComponents.CrcCalculator;

public class crc32Calculator {

    private static final int CRC_RESET_VALUE = 0xFFFFFFFF;

    private static final int[] CRC32_LOOKUP_TABLE = {
    ...
    public static long calculateCrc32(byte[] data) {
        return calculateCrc32(data, 0, data.length, CRC_RESET_VALUE);
    }

    public static long calculateCrc32(byte[] data, int offset, int length, long crc) {
        for (int i = offset; i < offset + length; i++) {
            crc = ((crc >>> 8) & 0x00FFFFFF) ^ CRC32_LOOKUP_TABLE[(int)((crc ^ data[i]) & 0xFF)];
        }
        return crc;
    }
    ...
    public static long calCrc32(byte[] data) {
        int flashStartAdd = 0;
        long crc32Value = CRC_RESET_VALUE;

        for (int i = 0; i < data.length / 4; i++) {
            crc32Value = calculateCrc32(data, flashStartAdd, 4, crc32Value);
            flashStartAdd += 4;
        }
